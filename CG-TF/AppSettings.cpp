...
void AppSettings::desenharArena() {
	...
	this->arena->desenhar(this->vision3d, this->getJogador()->corpo->getHeight());

}

void AppSettings::desenharPostoAbastecimento() {
	//glPushMatrix();
		//double xTranslated = this->postoAbastecimento->getX()/2;
		//double yTranslated = -this->postoAbastecimento->getY();
		//glTranslated(xTranslated, yTranslated, 0.0);
		//this->postoAbastecimento->desenharRectangle(0.75, 0.75, 0.75, true);
...
//	this->helicoptero->setarValores(this->getJogador());
	this->jogador->desenharHelicoptero(this->vision3d);
	this->jogador->mostrarTiros(this->vision3d);
	
	Helicoptero* inimigo;
	int i = 0;
	for (i = 0; i < this->quantidadeInimigos; i++) {
		inimigo = &(this->inimigos->at(i));
		inimigo->desenharHelicoptero(this->vision3d);
		inimigo->mostrarTiros(this->vision3d);
	}
}
...
void AppSettings::desenharObjetos() {	
	...
	// Se não acabar o combustível o jogo acaba
	if (this->jogador->temCombustivel == false) {
		this->mostrarTelaDeJogo = false;
		this->imprimirMensagem(false);
	}*/
...
void AppSettings::carregarInformacoesArena() {
	this->arena->setDadosArena(this->dadosArena);
}
...
void AppSettings::desenharCombustivel() {
	...
	glPushMatrix(); // Mover-se para metade da bateria
		...
		glTranslated(xTranslated, yTranslated, 0.0);		
		this->combustivel->desenharRectangle2d(this->combustivel->getHeight(), w, R, G, B, false);
	glPopMatrix();
	...
	glPushMatrix();
		...
		yTranslated = this->combustivel->getY();
		glTranslated(xTranslated, yTranslated, 0.0);
		this->combustivel->desenharContorno2d(0.0, 0.0, 0.0);
	glPopMatrix();
}
