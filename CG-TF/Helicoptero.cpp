...
void Helicoptero::setarValores(Circle* c) {
	...
	this->cz = this->dadosCircle->getR();
	this->posX = this->dadosCircle->getCx();
	this->posY = this->dadosCircle->getCy();
}

void Helicoptero::desenharMira(bool vision3d) {
	...
	// posicionar mira no meio do corpo
	float zTranslated = this->corpo->getHeight()/4;
	glTranslatef(xTranslated, yTranslated, zTranslated);

	if (vision3d) {

		this->mira->desenharRectangle3d(this->color[0], this->color[1],
				this->color[2], this->mira->getHeight()/2, true);
	} else {

		this->mira->desenharRectangle2d(this->mira->getHeight(),
...
void Helicoptero::desenharCorpo(bool vision3d) {
	float xTranslated = 0.0;
	float yTranslated = this->mira->getHeight();
	glPushMatrix();
	glTranslatef(xTranslated, yTranslated, 0.0);
	...
	if (vision3d) {
		...
		this->corpo->desenharRectangle3d(this->color[0], this->color[1],
				this->color[2], this->corpo->getHeight(), true);
	} else {

		this->corpo->desenharRectangle2d(this->corpo->getHeight(),
...
void Helicoptero::desenharCaudaDireita(bool vision3d) {
	...
	if (vision3d) {
		this->caudaDireita->desenharRectangle3d(this->color[0], this->color[1],
				this->color[2], this->caudaDireita->getHeight(), true);
	} else {

		this->caudaDireita->desenharRectangle2d(this->caudaDireita->getHeight(),
...
void Helicoptero::desenharCaudaEsquerda(bool vision3d) {
	float xTranslated = -(this->cauda->getWidth() / 2
	...
	if (vision3d) {
		this->caudaEsquerda->desenharRectangle3d(this->color[0], this->color[1],
				this->color[2], this->caudaEsquerda->getHeight(), true);
	} else {

		this->caudaEsquerda->desenharRectangle2d(
				this->caudaEsquerda->getHeight(),
...
void Helicoptero::desenharCaudaPrincipal(bool vision3d) {
	float xTranslated = 0.0;
	...
	if (vision3d) {
		this->cauda->desenharRectangle3d(this->color[0], this->color[1],
				this->color[2], this->cauda->getHeight(), true);
	} else {

		this->cauda->desenharRectangle2d(this->cauda->getHeight(),
...
void Helicoptero::aumentarGiroHelices() {
	...
	this->cz += this->velocidadeHelicoptero;
}
...
void Helicoptero::desenharHelices(Circle* c, bool vision3d) {
	glPushMatrix();
	...
	if (vision3d) {
		...
		for (i = 0; i < 4; i++) {
			...
			// Incluir dois pontos passando a altura como parâmetro
			Ponto auxP1 = aux.pontos->at(1);
			Ponto* p1 = new Ponto(auxP1.getX(), auxP1.getY());
			aux.pontos->push_back(*p1);

			Ponto auxP2 = aux.pontos->at(1);
			Ponto* p2 = new Ponto(auxP2.getX(), auxP2.getY());
			aux.pontos->push_back(*p2);

			aux.desenharPolyline3d(c, this->altura, 0.0, 0.0, 1.0);
...
void Helicoptero::desenharCentroHelice(bool vision3d) {
	float xTranslated = 0.0;
	float yTranslated = this->mira->getHeight() + 20; //this->corpo->getHeight();
	glPushMatrix();

	if (vision3d) {
		glTranslatef(xTranslated, yTranslated, this->corpo->getHeight() + 0.5);
		this->centroHelice->desenharCircle(1.0, 1.0, 0.0);
		this->desenharHelices(this->centroHelice, vision3d);
...
void Helicoptero::desenharHelicoptero(bool vision3d) {
	if (this->foiAtingido == true) {
		return;
	}

	bool realizarTransformacao = true;
	bool mostrarCirculo = false;
	...
	this->transformacao->definirTransformacao(fatorEscala,
			this->dadosCircle->getCx(), this->dadosCircle->getCy(), this->cz,
			this->dadosCircle->getR(), this->anguloGiro, this->anguloMira,
			this->mira->getHeight(), this->posX, this->posY);
	if (realizarTransformacao)
		this->transformacao->iniciarTransformacao(false, vision3d);

//	glScalef(this->fatorEscala, this->fatorEscala, this->fatorEscala);
//	glTranslated(this->dadosCircle->getCx(), this->dadosCircle->getCy(), 50);
	...
	if (mostrarCirculo) {
		glPushMatrix();
			...
			this->dadosCircle->desenharCircle(this->color[0], this->color[1],
					this->color[2]);
		glPopMatrix();
	}

	this->desenharMira(vision3d);
	this->desenharCorpo(vision3d);
	this->desenharCaudaPrincipal(vision3d);
	this->desenharCaudaDireita(vision3d);
	this->desenharCaudaEsquerda(vision3d);
	this->desenharCentroHelice(vision3d);
...
bool Helicoptero::detectarLimitesHelicopteros(
	...
	if (this->tipo == "jogador") {
		...
		if (jogador->enableMovimento == true
				&& this->detectarColisaoHelicoptero(jogador->dadosCircle)) {
			return false;
		}
		...
		for (i = 0; i < helicopterosInimigos->size(); i++) {
			h = &(helicopterosInimigos->at(i));
			if (h->foiAtingido == false
					&& h->identificador != this->identificador) {
				if (this->detectarColisaoHelicoptero(h->dadosCircle) == true) {
					//cout << "Helicoptero colidiu!\n";
					return false;
				}
...
void Helicoptero::rotacionarMira(float x, float y, bool vision3d) {
...
void Helicoptero::mostrarTiros(bool vision3d) {
	int i = 0;
	Tiro t;
	if (!this->tiros->empty()) {
		for (i = 0; i < (int) this->tiros->size(); i++) {
			t = tiros->at(i);
			t.desenharTiro(vision3d, this->mira->getHeight());
		}
	}
}
...
bool Helicoptero::verificaTiroInimigo(Helicoptero* helicopteroJogador) {
	...
	for (j = 0; j < this->tiros->size(); j++) {
		...
		if (helicopteroJogador->dadosCircle->internoCircunferencia(
				centro->getX(), centro->getY()) == true
	...
	for (i = 0; i < quantidadeInimigos; i++) {
		...
		for (j = 0; j < this->tiros->size(); j++) {
			...
			if (inimigo->foiAtingido == false) {
				...
				// Definindo pontos de verifica��o
				centro = new Ponto(t->getPosX(), t->getPosY());
				superior = new Ponto(t->getPosX(),
						t->getPosY() - t->tiro->getR());
				inferior = new Ponto(t->getPosX(),
						t->getPosY() + t->tiro->getR());
				direito = new Ponto(t->getPosX() + t->tiro->getR(),
						t->getPosY());
				esquerdo = new Ponto(t->getPosX() - t->tiro->getR(),
						t->getPosY());
...
int Helicoptero::resgatarObjeto(vector<ObjetoResgate>* objetosResgate) {
	...
	for (i = 0; i < (int) objetosResgate->size(); i++) {
		...
		if (obj->objetoResgatado == false
				&& obj->dadosObjetoResgate->internoCircunferencia(this->posX,
						this->posY) == true) {
...
void Helicoptero::movimentoAleatorio(float limiteSuperior, float limiteInferior,
		float limiteEsquerdo, float limiteDireito,
		vector<Helicoptero>* helicopterosInimigos, Helicoptero* jogador) {
	if (this->noLimiteDaArena) {
		this->anguloGiro += (rand() % 720);
	}
	this->moverParaFrente(limiteSuperior, limiteInferior, limiteEsquerdo,
			limiteDireito, helicopterosInimigos, jogador);
}
